...
class Config:
    ...
    mlp_hidden_dim = 1024                      # MLP head hidden dimension
    mlp_num_layers = 1                         # MLP head number of hidden layers
    ...
    # System parameters
    base_results_dir = './results_CIFAR100_standardMLP_kNN_monitoring'     # Name of the base directory to save checkpoints and other results
...
class StandardMLP(nn.Module):
    ...
    def forward(self, x):                                   # Standard MLP with Leaky ReLU activation
        z = x   
...
class MLP_ResNet(nn.Module):
    ...
    def __init__(self, resnet_embedding_dim, mlp_hidden_dim, mlp_num_layers, is_ablation=False):
        ...
        if not self.is_ablation:
            self.mlp = StandardMLP(resnet_embedding_dim, mlp_hidden_dim, mlp_num_layers) # Ablation study is run without the MLP head
...
def visualize_embeddings(embeddings, labels, output_dir, seed, name_prefix):
    ...
    plt.title(f"t-SNE of {name_prefix} Embeddings on CIFAR-100", fontsize=16)
    plt.xlabel("Component 1")
    plt.ylabel("Component 2")
    plt.grid(True, linestyle='--', alpha=0.6)
    filename = f"{name_prefix}_cifar100_tsne.png"
...
## --- CORRECTED --- This is the definitive, correct way to split the data to prevent overlap.
# 1. Create two base datasets from the same training data but with different transforms
train_dataset_aug = CIFAR100(root=args.data_dir, train=True, download=True, transform=train_transform)
train_dataset_no_aug = CIFAR100(root=args.data_dir, train=True, download=True, transform=test_transform)
...
print("\n--- Evaluating Pre-Trained ResNet Encoder (Baseline) on CIFAR-100 ---")
...
## --- CORRECTED --- Pass the correct 'gallery_loader' to the training function
...
if best_model_path and os.path.exists(best_model_path):
    ...
    model_name = "MLP_ResNet Ablation" if args.is_ablation else "MLP_ResNet"
